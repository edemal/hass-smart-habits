...
- smart_habits/accept_pattern: Accepts a pattern persistently via AcceptedPatternsStore
...
async def ws_accept_pattern(
    ...
    Accepts a pattern fingerprint persistently via AcceptedPatternsStore
    and refreshes the coordinator so the change is reflected immediately.
    The optional secondary_entity_id supports temporal sequence pattern acceptance.
    """
    entries = hass.config_entries.async_entries("smart_habits")
    if not entries:
        connection.send_error(msg["id"], "not_found", "Smart Habits integration not set up")
